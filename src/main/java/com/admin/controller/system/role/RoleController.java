...
public class RoleController extends BaseController {
    ...
    public ModelAndView list(Page page) throws Exception {
        ...
        List<Role> roleList = roleService.listAllRoles(pd); // 列出所有部门
        ...
        try {
            if (Jurisdiction.buttonJurisdiction(menuUrl, "edit")) {
                if (null != menuIds && !"".equals(menuIds.trim())) {
                    ...
                    pd.put("value", rights.toString());//查询权限
                } else {
                    Role role = new Role();
                    role.setRIGHTS("");
                ...
                pd.put("ROLE_ID", ROLE_ID);
                roleService.updateQx("cha_qx", pd);//查询权限
     ...
     * 保存角色菜单权限
     */
    @RequestMapping(value = "/auth/baseSave")
    public void baseSave(@RequestParam String ROLE_ID, @RequestParam String menuIds, PrintWriter out) throws Exception {
        PageData pd = new PageData();
        try {
            Role role = roleService.getRoleById(ROLE_ID);
            if (null != menuIds && !"".equals(menuIds.trim())) {
                BigInteger baseQx = RightsHelper.sumRights(Tools.str2StrArray(menuIds));
                role.setBASE_QX(baseQx.toString());
                roleService.updateRoleBaseQx(role);
            } else {
                role.setBASE_QX("");
                role.setROLE_ID(ROLE_ID);
                roleService.updateRoleBaseQx(role);
...
//					List<PageData> appuserlist = roleService.listAllAppUByRid(pd);
    ...
    public String authManage(Model model) throws Exception {
     ...
     * 查看用户权限页面
     */
    @RequestMapping(value = "/userQxCheck")
    public String userQxCheck(@RequestParam String MENU_ID,Model model) throws Exception {
//        List<Role> roleList = roleService.listAllRoles(); // 列出所有部门
//        List<Role> roleList_z = roleService.listAllRolesByPId(pd); // 列出此部门的所有下级
//
//        List<PageData> kefuqxlist = roleService.listAllkefu(pd); // 管理权限列表
//        List<PageData> gysqxlist = roleService.listAllGysQX(pd); // 用户权限列表
        List<Role> roleList = roleService.listAllRolesNew(MENU_ID,this.getPageData());// 列出所有部门
        try {
            JSONArray arr = JSONArray.fromObject(roleList);
            String json = arr.toString();
            json = json.replaceAll("ROLE_ID", "id").replaceAll("ROLE_NAME", "name").replaceAll("subRole", "nodes").replaceAll("hasRole", "checked");
            model.addAttribute("zTreeNodes", json);
            model.addAttribute("menuId", MENU_ID);
        } catch (Exception e) {
            logger.error(e.toString(), e);
        }
        return "authorizationUserQxCheck";
    }
