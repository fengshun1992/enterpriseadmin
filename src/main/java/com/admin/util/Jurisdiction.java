			...
			for (int j = 0; j < menuList.get(i).getSubMenu().size(); j++) {
				if (menuList.get(i).getSubMenu().get(j).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])) {
					if (!menuList.get(i).getSubMenu().get(j).isHasMenu()) { // 判断有无此菜单权限
						...
						String [] adds =  map.get("adds").toString().split(",");
						String add = "";
						for (String adds2 : adds) {
							add = RightsHelper.testRights(adds2, MENU_ID) || isAdmin ? "1" : "0";
							if(add.equals("1")){
								break;
							}
						}
						map.put("add",add);
						String [] dels =  map.get("dels").toString().split(",");
						String del = "";
						for (String dels2 : dels) {
							del = RightsHelper.testRights(dels2, MENU_ID) || isAdmin ? "1" : "0";
							if(del.equals("1")){
								break;
							}
						}
						map.put("del",del);
						String [] edits =  map.get("edits").toString().split(",");
						String edit = "";
						for (String edits2 : edits) {
							edit = RightsHelper.testRights(edits2, MENU_ID) || isAdmin ? "1" : "0";
							if(edit.equals("1")){
								break;
							}
						}
						map.put("edit",edit);
						String [] chas =  map.get("chas").toString().split(",");
						String cha = "";
						for (String chas2 : chas) {
							cha = RightsHelper.testRights(chas2, MENU_ID) || isAdmin ? "1" : "0";
							if(cha.equals("1")){
								break;
							}
						}
				...
				if (menuList.get(i).getSubMenu().get(j).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])) {
					if (!menuList.get(i).getSubMenu().get(j).isHasMenu()) { // 判断有无此菜单权限
						...
						if ("add".equals(type)) {
							...
							boolean add = false;
							for (String adds2 : adds) {
								add = RightsHelper.testRights(adds2, MENU_ID) || isAdmin;
								if(add==true){
									break;
								}
							}
							...
							boolean del = false;
							for (String dels2 : dels) {
								del = RightsHelper.testRights(dels2, MENU_ID) || isAdmin;
								if(del==true){
									break;
								}
							}
							...
							boolean edit = false;
							for (String edits2 : edits) {
								edit = RightsHelper.testRights(edits2, MENU_ID) || isAdmin;
								if(edit==true){
									break;
								}
							}
							...
							boolean cha = false;
							for (String chas2 : chas) {
								cha = RightsHelper.testRights(chas2, MENU_ID) || isAdmin;
								if(cha==true){
									break;
								}
							}
