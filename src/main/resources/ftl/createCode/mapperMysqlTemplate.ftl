...
<mapper namespace="${objectName}Mapper">
	...
	<#list fieldList as var>
			...
			${objectNameUpper}_ID,owner
		) values (
	<#list fieldList as var>
			${r"#{"}${var[0]}${r"}"},	
	</#list>
			${r"#{"}${objectNameUpper}_ID${r"}"},${r"#{"}OWNER${r"}"}
		)
	</insert>
	...
	<delete id="delete" parameterType="pd">
		...
		where 
			${objectNameUpper}_ID = ${r"#{"}${objectNameUpper}_ID${r"}"} and owner=${r"#{"}OWNER${r"}"}
	</delete>
	...
	<select id="datalistPage" parameterType="page" resultType="pd">
		...
		from 
				${tabletop}${objectNameUpper} a  where owner=${r"#{"}pd.OWNER${r"}"}
	</select>
	...
	<select id="listAll" parameterType="pd" resultType="pd">
		...
		<#list fieldList as var>
				...
				a.${objectNameUpper}_ID
		from 
				${tabletop}${objectNameUpper} a  where owner=${r"#{"}OWNER${r"}"}
	</select>
