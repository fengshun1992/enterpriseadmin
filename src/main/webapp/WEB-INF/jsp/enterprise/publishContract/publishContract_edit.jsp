...
<head>
    ...
    <script type="text/javascript">
        ...
        function checkContractNo() {
            ...
            $.ajax({
                type: "post",
                url: '/publishContract2/checkContractNo.do',
        ...
        function save() {
            var array = new Array();
            <c:forEach items="${pd.fileParamsList_1 }" var="t">
            array.push(${t.paramId });
            </c:forEach>
            <c:forEach items="${pd.fileParamsList_2 }" var="t">
            array.push(${t.paramId });
            </c:forEach>
            <c:forEach items="${pd.fileParamsList_3 }" var="t">
            array.push(${t.paramId });
            </c:forEach>
            <c:forEach items="${pd.fileParamsList_4 }" var="t">
            array.push(${t.paramId });
            </c:forEach>
...
<form action="publishContract2/${msg }.do" name="form" id="form" method="post" enctype="multipart/form-data">
    <input type="hidden" name="templateId" id="templateId" value="${templateId }"/>
    <div id="zhongxin">
        <table id="table_report" class="table table-striped table-bordered table-hover">
            <tr>
                <td style="width:100px;text-align: right;padding-top: 13px;">合同名称:</td>
                <td><input type="text" name="contractName" id="contractName" value="${contract.contractName }"
                           maxlength="32"
            ...
            <c:forEach items="${pd.fileParamsList_1 }" var="item">
                <tr>
                    <td style="width:100px;text-align: right;padding-top: 13px;">${item.param }</td>
                    <td><input type="text" name="_${item.paramId }" id="_${item.paramId }" value="${item.value }"
                               maxlength="32"
                               title="参数值"/>
                    </td>
                </tr>
            ...
            <c:forEach items="${pd.fileParamsList_2 }" var="item">
                <tr>
                    <td style="width:100px;text-align: right;padding-top: 13px;">${item.param }</td>
                    <td><input type="text" name="_${item.paramId }" id="_${item.paramId }" value="${item.value }"
                               maxlength="32"
            ...
            <c:forEach items="${pd.fileParamsList_3 }" var="item">
                <tr>
                    <td style="width:100px;text-align: right;padding-top: 13px;">${item.param }</td>
                    <td><input type="text" name="_${item.paramId }" id="_${item.paramId }" value="${item.value }"
                               maxlength="32"
            ...
            <c:forEach items="${pd.fileParamsList_4 }" var="item">
                <tr>
                    <td style="width:100px;text-align: right;padding-top: 13px;">${item.param }</td>
                    <td><input type="text" name="_${item.paramId }" id="_${item.paramId }" value="${item.value }"
                               maxlength="32"
