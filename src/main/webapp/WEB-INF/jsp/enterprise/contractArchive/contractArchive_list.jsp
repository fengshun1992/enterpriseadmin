...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
		...
		<!-- 检索  -->
		<form action="contractArchive/list.do" method="post" name="form" id="form">
			<table>
				<tr>
					<td>
				...
				<c:choose>
					<c:when test="${not empty contractList}">
						...
						<c:forEach items="${contractList}" var="item" varStatus="vs">
							...
							<td>
								<c:if test="${item.type == 1}">发行中</c:if>
								<c:if test="${item.type == 2}">已下架</c:if>
							</td>
...
<div style="width: 100%;height: 800px">
	<iframe id="frame" style="width: 100%;height: 800px" frameborder="no"></iframe>
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '/contractArchive/goAdd.do';
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function edit(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '/contractArchive/goEdit.do?ID='+ID;
			diag.Width = 800;
			diag.Height = 300;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "/contractArchive/delete.do?ID="+ID;
