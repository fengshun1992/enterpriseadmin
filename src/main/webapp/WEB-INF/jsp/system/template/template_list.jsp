				...
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="SMS_KEY" value="${pd.SMS_KEY}" placeholder="这里输入关键字" />
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">短信模板</th>
						<th class="center">短信类型备注</th>
						<th class="center">发送短信通道类型</th>
						<th class="center">添加时间</th>
						<th class="center">更新时间</th>
						<th class="center">创建者</th>
						<th class="center">修改者</th>
						<th class="center">短信运营商备注</th>
						<th class="center">短信发送限制次数</th>
						<th class="center">短信签名</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<%--<td class='center' style="width: 30px;">${vs.index+1}</td>--%>
										...
										<td style="width: 500px;">${var.SMS_TEMPLATE}</td>
										<td>${var.SMS_REMARK}</td>

										<td>${var.SMS_SEND_TYPE}</td>
										<td>${var.ADD_TIMD}</td>
										<td>${var.UPDATE_TIME}</td>
										<td>${var.CREATER}</td>
										<td>${var.CHANGER}</td>
										<td>${var.SMS_KEY}</td>
										<td class='center'>${var.SMS_TIMES}</td>
										<td>${var.SMS_SIGN}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "/template/delete.do?TEMPLATE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '/template/goEdit.do?TEMPLATE_ID='+Id;
			diag.Width=600;
			diag.Height=530;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '/template/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='/template/excel.do';
		}
		</script>
