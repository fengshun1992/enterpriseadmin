		...
		<div id="sidebar">
				...
				<div id="sidebar-shortcuts">
					...
					<%--<button class="btn btn-small btn-success" onclick="changeMenu();" title="切换菜单"><i class="icon-pencil"></i></button>--%>
						...
						<button class="btn btn-small btn-info" title="UI实例" id="adminUI" <%--onclick="window.open('/static/UI_new');"--%>><i class="icon-eye-open"></i></button>
					</div>
					  ...
					  <ul class="submenu">
							<c:forEach items="${menu.subMenu}" var="sub">
								...
								<c:choose>
									...
									<li id="z${sub.MENU_ID }">
										<c:choose>
											<c:when test="${sub.MENU_URL == 'menu.do'}">
												<a style="cursor:pointer;" target="mainFrame"  onclick="menu()"><i class="icon-double-angle-right"></i>${sub.MENU_NAME }</a></li>
											</c:when>
											<c:otherwise>
