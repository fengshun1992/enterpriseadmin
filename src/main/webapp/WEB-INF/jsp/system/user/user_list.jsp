			...
			<table>
				<tr>
					...
					<td style="vertical-align:top;">
						<select class="chzn-select" name="ROLEZ_ID" id="rolez_id" data-placeholder="请选择组" style="vertical-align:top;width: 120px;" onchange="selectRole(this.value)">
							<option value=""></option>
							<option value="">全部</option>
							<c:forEach items="${roleList}" var="role">
								<option value="${role.ROLE_ID }" <c:if test="${pd.ROLEZ_ID==role.ROLE_ID}">selected</c:if>>${role.ROLE_NAME }</option>
							</c:forEach>
						</select>
					</td>

					<%--<td style="vertical-align:top;display: none">
					 	<select name="ROLE_ID" id="role_id" style="width: 120px;background: #fafafa;height: 28px;line-height: 26px;font-size: 9px;">
							<option style="font-size:14px;" value="" disabled selected>请选择职位</option>
							<option value="" style="font-size:13px">全部</option>
							<c:forEach items="${roles}" var="role">
								<option style="font-size:13px" value="${role.ROLE_ID }" <c:if test="${pd.ROLE_ID==role.ROLE_ID}">selected</c:if>>${role.ROLE_NAME }</option>
							</c:forEach>
					  	</select>
					</td>--%>
					<td style="vertical-align:top;">
						<select class="chzn-select" name="IS_LOGOUT" id="IS_LOGOUT" data-placeholder="请选择是否注销" style="vertical-align:top;width: 150px;">
							<option value=""></option>
							<option value="">全部</option>
							<option value="0" <c:if test="${pd.IS_LOGOUT=='0'}">selected</c:if>>可使用</option>
							<option value="1" <c:if test="${pd.IS_LOGOUT=='1'}">selected</c:if>>不可用</option>
						</select>
					</td>
					...
					<c:if test="${QX.cha == 1 }">
						...
						<td style="vertical-align:top;">
							<a class="btn btn-mini btn-light" onclick="window.location.href='/user/listtabUsers.do';" title="切换模式">
					...
					<c:when test="${not empty userList}">
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${user.IS_LOGOUT == '可用'}">
											<span class="badge badge-success">${user.IS_LOGOUT}</span>
										</c:when>
										<c:otherwise>
											<span class="badge badge-important">${user.IS_LOGOUT}</span>
										</c:otherwise>
									</c:choose>
								</td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											...
											<c:if test="${user.USERNAME == 'admin'}">
												<a class='btn btn-mini btn-info' title="不能编辑"><i class='icon-edit'></i></a>
											</c:if>
										</c:if>
										...
										<c:if test="${QX.edit == 1 }">
											<c:choose>
												<c:when test="${user.IS_LOGOUT=='可用'}">
													<c:if test="${user.USERNAME != 'admin'}">
														<a class='btn btn-mini btn-warning' title="注销"
														   onclick="logOutUser('${user.USER_ID }','${user.USERNAME }','1');">
															<i class='icon-off'></i></a>
													</c:if>
													<c:if test="${user.USERNAME == 'admin'}">
														<a class='btn btn-mini btn-warning' title="不能注销">
															<i class='icon-off'></i></a>
													</c:if>
												</c:when>
												<%--<c:otherwise>
													<c:if test="${user.USERNAME != 'admin'}">
														<a class='btn btn-mini btn-success' title="开启"
														   onclick="logOutUser('${user.USER_ID }','${user.USERNAME }','0');">
															<i class='icon-off'></i></a>
													</c:if>
													<c:if test="${user.USERNAME == 'admin'}">
														<a class='btn btn-mini btn-success' title="不能注销">
															<i class='icon-off'></i></a>
													</c:if>
												</c:otherwise>--%>
											</c:choose>
...
//		})
		...
		//注销
		function logOutUser(user_id,msg,is_logout){
			if(is_logout == '1'){
				bootbox.confirm("确定要注销["+msg+"]吗?", function(result) {
					if(result) {
						top.jzts();
						var url = "/user/logOutUser.do?USER_ID="+user_id+"&IS_LOGOUT="+is_logout+"&tm="+new Date().getTime();
				...
				bootbox.confirm("确定要开启["+msg+"]吗?", function(result) {
					if(result) {
						top.jzts();
						var url = "/user/logOutUser.do?USER_ID="+user_id+"&IS_LOGOUT="+is_logout+"&tm="+new Date().getTime();
						$.get(url,function(data){
		...
		function fromExcel(){
			...
			function selectRole(id){
				$.ajax({
					type: "POST",
					url: '/user/getAllRoles.do?ROLE_ID='+id,
					dataType:'json',
					success: function(data){
						$("#role_id").val("");
						$("#role_id").text("");
						$("#role_id").append('<option value="" style="font-size:13px">全部 </option>');
						for (var i = 0; i < data.length; i++) {
							var role = data[i];
							$("#role_id").append('<option value="' + role.ROLE_ID + '" style="font-size:13px">' + role.ROLE_NAME + '</option>');
						}
					}
				});
