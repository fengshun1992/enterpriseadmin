...
<script type="text/javascript">
	...
	function editTb(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑图标";
		 diag.URL = '/menu/toEditicon.do?MENU_ID='+menuId;
		 diag.Width = 530;
		 diag.Height = 150;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function openClose(menuId,curObj,trIndex){
		...
		if(txt=="展开"){
			...
			var url = "/menu/sub.do?MENU_ID="+menuId+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data.length>0){
					var html = "";
					$.each(data,function(i){
						...
						html += "<td><c:if test="${QX.edit == 1 }"><a class='btn btn-mini btn-info' title='编辑' onclick='editmenu(\""+this.MENU_ID+"\")'><i class='icon-edit'></i></a></c:if>"
                        html += "<c:if test="${QX.del == 1 }"><a class='btn btn-mini btn-danger' title='删除' onclick='delmenu(\""+this.MENU_ID+"\",false)'><i class='icon-trash'></i></a></c:if></td>";
						html += "</tr>";
		...
		<div>
			<c:if test="${QX.add == 1 }">
				<table style="width:100%;">
					<tr>
						<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="addmenu();">新增</a></td>
					</tr>
				</table>
			</c:if>
			&nbsp;&nbsp;
